...
async getAllSuggests(id) {
    const result = await client.query(`SELECT *,  to_char_french(created_at, 'TMDay DD TMMonth YYYY') as suggest_date FROM "suggest" WHERE "nanny_id" = $1 ORDER BY "created_at" DESC`, [id]);
    return result.rows;
};
